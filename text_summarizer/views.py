...
def index(request):
    return render(request, "text-summarizer-index.html")

def text_summarizer(request):
    pass
# def text_summarizer(request):
#     if request.POST.get('long_text', None) is not None and request.META.get('HTTP_X_REQUESTED_WITH') == 'XMLHttpRequest':
#         print("in func")
...
#         return JsonResponse(summary_text,safe=False)
#     else:
